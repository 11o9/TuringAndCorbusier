...
namespace TuringAndCorbusier
{
    class AG3 : ApartmentmentGeneratorBase
    {
        public override ApartmentGeneratorOutput generator(Plot plot, ParameterSet parameterSet, Target target)
        {
            ...
            if (!valid)
                return new ApartmentGeneratorOutput();
            ...
            //building outlines
            List<List<Curve>> buildingOutlines = buildingOutlineMakerAG3(midlinePolyline, width);

            ////////////////////////////////////////////////////////////////////////////////////////

            //parking lot
            ParkingLotOnEarth parkingLotOnEarth = new ParkingLotOnEarth(parkingLotMaker(coreProperties, parameterSet, plot.SimplifiedBoundary));
            ParkingLotUnderGround parkingLotUnderGround = new ParkingLotUnderGround();

            ApartmentGeneratorOutput result = new ApartmentGeneratorOutput("PT-2", plot, buildingType, parameterSet, target, coreProperties, householdProperties,parkingLotOnEarth, parkingLotUnderGround, buildingOutlines, new List<Curve>());
            return result;
        ...
        private double[] GAparameterset = { 0.8, 0.05, 1, 1, 1, 10, 1 };
        ...
        private List<List<Curve>> buildingOutlineMakerAG3(Polyline midline, double width)
        {
            List<List<Curve>> buildingOutlines = new List<List<Curve>>();
            List<Curve> buildingOutlinesB = new List<Curve>();

            List<Point3d> outPoints = new List<Point3d>(midline);
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                tempPoint.Transform(Transform.Translation(Vector3d.Multiply(xx, width)));
                tempPoint.Transform(Transform.Translation(Vector3d.Multiply(yy, width)));
