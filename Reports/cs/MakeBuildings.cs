    ...
    public class MakeBuildings
    {
        public static List<List<Brep>> makeBuildings(ApartmentGeneratorOutput agOut, bool drawComplexModeling)
        {
            ...
            for (int i = 0; i < coreOutline.Count; i++)
            {
                ...
                {
                    Surface tempSurface = Surface.CreateExtrusion(coreOutline[i][j], Vector3d.Multiply(Vector3d.ZAxis, storiesHeight * (stories + 1) + pilotiHeight));
                    Brep tempBrep = tempSurface.ToBrep().CapPlanarHoles(1);
            ...
            for (int i = 0; i < aptLines.Count; i++)
            {
                if (agOut.Target.TargetArea[agOut.HouseholdProperties[i][0][0].HouseholdSizeType] * 1000 * 1000 < Consts.AreaLimit)
                {
                    for (int j = 0; j < agOut.HouseholdProperties[i].Count; j++)
                    {
                        ...
                        Point3d pt2 = aptLines[i].PointAtStart;
                        ...
                        Brep corridorFloorBrep = Surface.CreateExtrusion(lineTemp.ToNurbsCurve(), -verticalVec * Consts.corridorWidth).ToBrep();
