...
namespace TuringAndCorbusier
{
    ...
    {
        //Constructor, 생성자
        public FloorPlan(HouseholdProperties householdProperty, List<FloorPlanLibrary> fpls, double exWallThickness, double inWallThickness, double entDoorWidth, double inDoorWidth)
        {
            FloorPlanLibrary fpl = typeDetector(fpls, householdProperty);

            List<List<Curve>> wallCurves; List<List<Curve>> tilingCurves; List<List<Curve>> doorCurves; List<List<Curve>> windowCurves;
            planDrawer(fpl, householdProperty, exWallThickness, inWallThickness, entDoorWidth, inDoorWidth, out wallCurves, out tilingCurves, out doorCurves, out windowCurves);
            ...
            List<List<Curve>> centerLines; List<Dimension> dimensions;
            ...
            List<List<Curve>> balconyLines = balconyLineMaker(householdProperty, exWallThickness);

            List<Curve> coreDetails = coreDetailMaker(householdProperty, exWallThickness);
            List<List<Curve>> tempCore = new List<List<Curve>>();
            tempCore.Add(coreDetails);
            coreDetails = totalTransformation(tempCore, householdProperty)[0];
            ...
            List<Curve> caps = capsMaker(householdProperty, exWallThickness, inWallThickness);
            List<Rhino.Display.Text3d> roomTags = GetRoomTag(householdProperty, fpl);
        ...
        public FloorPlan(HouseholdProperties householdProperty, List<FloorPlanLibrary> fpls)
        {
            FloorPlanLibrary fpl = typeDetector(fpls, householdProperty);
        ...
        public List<Curve> coreDetails { get; private set; }
        public List<Dimension> dimensions { get; private set; }
        public List<Curve> caps { get; private set; }
        public List<Rhino.Display.Text3d> roomTags { get; private set; }

        //Method, 메소드
        public List<Rhino.Display.Text3d> GetRoomTag(HouseholdProperties tempHouseholdProperties, FloorPlanLibrary fpl)
        {
            ...
            {
                Rhino.Display.Text3d tempText3d = new Rhino.Display.Text3d(fpl.tagNameList[i], new Plane(remappedPoints[i], Vector3d.ZAxis), 30);
        ...
        private FloorPlanLibrary typeDetector(List<FloorPlanLibrary> plantypes, HouseholdProperties householdproperty)
        {
            ...
            for (int i = 0; i < plantypes.Count; i++)
            {
                ...
                //isBigger
                bool isBigger;
                double dXa = (householdproperty.XLengthA - householdproperty.XLengthB) - (plantype.xLengthA - plantype.xLengthB);
                double dXb = householdproperty.XLengthB - plantype.xLengthB;
                double dYa = (householdproperty.YLengthA - householdproperty.YLengthB) - (plantype.yLengthA - plantype.yLengthB);
                double dYb = householdproperty.YLengthB - plantype.yLengthB;
                ...
                //priority
                if (hhpIsCorner == plantype.isCorner && isBigger && isYBequal)
                    priority.Add(0);
                else if (hhpIsCorner == plantype.isCorner && isYBequal)
                    priority.Add(1);
                else
                    priority.Add(2);
        ...
        private void planDrawer(FloorPlanLibrary fpl, HouseholdProperties hhp, double exWallThickness, double inWallThickness, double entDoorWidth, double inDoorWidth, out List<List<Curve>> wallCurves, out List<List<Curve>> tilingCurves, out List<List<Curve>> doorCurves, out List<List<Curve>> windowCurves)
        {
        ...
        public class Dimension
        {
            ...
            public Dimension(List<Point3d> endPoints, Point3d side, double length)
            {
                ...
                Plane pln = new Plane((dimensionLine.PointAtStart + dimensionLine.PointAtEnd) / 2, xVec, yVec);
                double fifties = line.Length - line.Length % 50;
                Rhino.Display.Text3d num = new Rhino.Display.Text3d(fifties.ToString(), pln, 200);
            ...
            Rhino.Display.Text3d numberText;

            //Methods

            //Properties
            public Curve DimensionLine { get { return this.dimensionLine; } }
            public List<Curve> ExtensionLine { get { return this.extensionLine; } }
            public Rhino.Display.Text3d NumberText { get { return this.numberText; } }
        }
        ...
        private void centerLinesAndDimensions(List<Polyline> allOutlines, Polyline outline, double extraLength, out List<List<Curve>> centerLines, out List<Dimension> dimensions)
        {
            ...
            for (int i = 0; i < xAxisP.Count - 1; i++)
            {
                ...
                dimensions.Add(new Dimension(endPts, new Point3d(posX[i], yMin, 0), 2000));
            }
            for (int i = 0; i < yAxisP.Count - 1; i++)
            {
                ...
                dimensions.Add(new Dimension(endPts, new Point3d(xMax, posY[i], 0), 2000));
            }
        ...
        private List<Curve> coreDetailMaker(HouseholdProperties hhp, double exWallThickness)
        {
            double w = exWallThickness;
            double elevEnt = 500;
            double elevH = 1700;
            double elevW = 3050;
            double cor = 1250;
            ...
            Point3d pt = Point3d.Origin;
            pt = move(pt, -x * w + y * cor); outer.Add(pt);
            pt = move(pt, y * (2500 + w)); outer.Add(pt);
            pt = move(pt, x * 3050); outer.Add(pt);
            pt = move(pt, y * 1000); outer.Add(pt);
            pt = move(pt, x * (2500 + 2 * w)); outer.Add(pt);
            pt = move(pt, -y * (3500 + w)); outer.Add(pt);

            Polyline outerPoly = new Polyline(outer);

            //inner
            List<Point3d> inner = new List<Point3d>();
            pt = Point3d.Origin;
            pt = move(pt, y * cor); inner.Add(pt);
            pt = move(pt, y * 500); inner.Add(pt);
            pt = move(pt, x * elevEnt); inner.Add(pt);
            pt = move(pt, y * w); inner.Add(pt);
            pt = move(pt, -x * elevEnt); inner.Add(pt);
            pt = move(pt, y * (2000 - w)); inner.Add(pt);
            pt = move(pt, x * (elevW - w)); inner.Add(pt);
            pt = move(pt, -y * (2000 - w)); inner.Add(pt);
            pt = move(pt, -x * elevEnt); inner.Add(pt);
            pt = move(pt, -y * w); inner.Add(pt);
            pt = move(pt, x * elevEnt); inner.Add(pt);
            pt = move(pt, -y * 500); inner.Add(pt);
            pt = move(pt, x * w); inner.Add(pt); Point3d stairStart = new Point3d(pt);
            pt = move(pt, y * (250 * 9 + cor)); inner.Add(pt);
            pt = move(pt, x * cor * 2); inner.Add(pt);
            pt = move(pt, -y * (250 * 9 + cor)); inner.Add(pt);

            Polyline innerPoly = new Polyline(inner);

            //stairs
            List<Curve> stairs = new List<Curve>();
            Point3d stairEnd = move(stairStart, x * 2500);
            Line line = new Line(stairStart, stairEnd);
            for (int i = 0; i < 10; i++)
            {
                Curve temp = line.ToNurbsCurve();
                temp.Translate(y * 250 * i);
                stairs.Add(temp);
            }
            Point3d stairMidStart = move(stairStart, x * 2500 / 2);
            Point3d stairMidEnd = move(stairMidStart, y * 250 * 9);
            stairs.Add(new Line(stairMidStart, stairMidEnd).ToNurbsCurve());

            //elevator
            List<Curve> elevCurve = new List<Curve>();
            List<Point3d> elevTemp = new List<Point3d>();
            pt = move(inner[4], x * 50 + y * 50); elevTemp.Add(pt);
            pt = move(inner[5], x * 50 - y * 50); elevTemp.Add(pt);
            pt = move(inner[6], -x * 50 - y * 50); elevTemp.Add(pt);
            pt = move(inner[7], -x * 50 + y * 50); elevTemp.Add(pt);
            pt = move(inner[4], x * 50 + y * 50); elevTemp.Add(pt);
            elevCurve.Add(new Polyline(elevTemp).ToNurbsCurve());
            elevTemp.Clear();
            pt = move(inner[4], x * 50 + y * 50); elevTemp.Add(pt);
            pt = move(inner[6], -x * 50 - y * 50); elevTemp.Add(pt);
            elevCurve.Add(new Polyline(elevTemp).ToNurbsCurve());
            ...
            List<Curve> output = new List<Curve>();
            output.Add(outerPoly.ToNurbsCurve());
            output.Add(innerPoly.ToNurbsCurve());
            output.AddRange(stairs);
            output.AddRange(elevCurve);

            if (Vector3d.CrossProduct(hhp.XDirection, hhp.YDirection).Z > 0)
            {
                output = mirrorYZ(output, true);
                output = translator(output, Vector3d.YAxis * 300);
                return output;
            }
            else
            {
        ...
        private Point3d move(Point3d pt, Vector3d vec)
        {
            Point3d temp = new Point3d(pt);
            temp.Transform(Transform.Translation(vec));
            return temp;
        }
        private List<Curve> mirrorYZ(List<Curve> curves, bool doMirror)
        {
            List<Curve> output = new List<Curve>();
            if (doMirror)
            {
                for (int i = 0; i < curves.Count; i++)
                {
                    ...
                    {
                        Curve temp = curves[i].DuplicateCurve();
                        temp.Transform(Transform.Mirror(Plane.WorldYZ));
                        output.Add(temp);
                    }
                }
            }
